...
public class StreamClient(HttpClient http)
{
    ...
    )
    {
        ...
        return _cipherManifest =
            ...
            ?? throw new YoutubeReExplodeException("Failed to extract the cipher manifest.");
    }
    ...
    )
    {
        foreach (var streamData in streamDatas)
        {
            var itag =
                streamData.Itag
                ?? throw new YoutubeReExplodeException("Failed to extract the stream itag.");

            var url =
                streamData.Url
                ?? throw new YoutubeReExplodeException("Failed to extract the stream URL.");
            ...
            var container =
                ...
                ?? throw new YoutubeReExplodeException("Failed to extract the stream container.");

            var bitrate =
                streamData.Bitrate?.Pipe(s => new Bitrate(s))
                ?? throw new YoutubeReExplodeException("Failed to extract the stream bitrate.");
            ...
            {
                throw new YoutubeReExplodeException("Failed to extract the stream codec.");
            }
        }
    }
